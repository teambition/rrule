...
RRuleSet.prototype = {
    ...
    */
    rdate: function (rdate) {
        if (!contains(this._rdate.map(Number), Number(rdate))) {
    ...
    */
    exdate: function (exdate) {
        if (!contains(this._exdate.map(Number), Number(exdate))) {
    ...
    */
    ...
    _iter: function (iterResult) {
        ...
        iterResult.add = function (date) {
            var dt = Number(date)
            if (!_exdateHash[dt]) {
                evalExdate(new Date(dt - 1), new Date(dt + 1));
                if (!_exdateHash[dt]) {
                    _exdateHash[dt] = true;
                    return _addResult.call(this, date);
                }
            }
            return true;
        }
